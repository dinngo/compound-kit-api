...
describe('Transaction: Collateral Swap', function () {
  ...
  let supplyAmount: common.TokenAmount;
  let borrowAmount: common.TokenAmount;
  ...
  it('user collateral swap Wrapped Native Token in USDC market', async function () {
    ...
    supplyAmount = new common.TokenAmount(collateralToken, collateralTokenInitBalance);
    await utils.supply(chainId, user, marketId, supplyAmount);

    // 2. user has borrowed 2000 USDC
    borrowAmount = new common.TokenAmount(baseToken, baseTokenBorrowAmount);
